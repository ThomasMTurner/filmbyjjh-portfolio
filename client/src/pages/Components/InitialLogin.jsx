...
export function AdminForm (props) {
    const [buttonColours, setButtonColours] = useState({'tick': 'white', 'back': 'white'});
    ...
    return (
      ...
      <div style={{display:'flex', flexDirection:'row', alignItems:'center'}}>
        <button style={{all:'unset', position:'relative', right:'1rem'}} onMouseEnter={() => setButtonColours(prevState => ({ ...prevState, back: 'silver' }))} onMouseLeave={() => setButtonColours(prevState => ({ ...prevState, back: 'white' }))} onClick={() => props.setAdminLogin(false)}>
        ...
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0}} transition={{ duration: 1.2 }} viewport={{ once: true }}>
          <div className='initial-login-container' id='initial-login-container'>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1rem', position: 'relative', bottom: '3rem' }}>
                <p style={{ fontSize: '2.5rem', fontFamily: 'helvetica', fontWeight: '300', color: 'white', position:'relative', bottom:'2rem' }}> Welcome to <span style={{ fontWeight: 'bold', textDecoration: 'underline', textDecorationThickness: '0.2rem' }}>FILM BY JJH</span>. </p>
