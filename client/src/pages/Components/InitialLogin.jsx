...
export function AdminForm (props) {
    ...
    return (
      ...
      <form onChange={(event) => handleAdminFormChange(event)} className='admin-login-form'>
        ...
        {!isSubmitting.checkToken && <MdCheck className='admin-login-button' color={buttonColours.tick} size={40} onClick={() => handleSubmit()} onMouseEnter={() => setButtonColours(prevState => ({ ...prevState, tick: 'green' }))} onMouseLeave={() => setButtonColours(prevState => ({ ...prevState, tick: 'silver' }))}/>}
        ...
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0}} transition={{ duration: 1.2 }} viewport={{ once: true }}>
          <div className='initial-login-container' id='initial-login-container'>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1rem', position: 'relative', bottom: '3rem' }}>
                <p style={{ fontSize: '2.5rem', fontFamily: 'helvetica', fontWeight: '300', color: 'black', position:'relative', bottom:'2rem' }}> Welcome to <span style={{ color:'black', fontWeight: 'bold', textDecoration: 'underline' }}>FILM BY JJH</span>. </p>
