...
import { BsArrowRight } from 'react-icons/bi';
...
const PortfolioCard = (props) => {
    ...
    return (
        <div className='portfolio-card'>
            <p>{props.title}</p>
            <button className='portfolio-card-button' type="button" onClick={() => navigate(`/portfolio/${props.title.toLowerCase().slice(0, 5)}`)}>View more <BiArrowToRight color={'blue'} size={20} /></button>
            <p>{props.desc}</p>
        </div>
    )
}
