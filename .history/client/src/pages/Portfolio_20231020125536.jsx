...
const PortfolioCard = (props) => {
    ...
    return (
        <div className='portfolio-card'>
            <p style={{fontFamily:'helvetica', fontWeight:'200', fontSize:'1.5rem', color:'black'}}>{props.title}</p>
            <button className='portfolio-card-button' type="button" onClick={() => navigate(`/portfolio/${props.title.toLowerCase().slice(0, 5)}`)}>View more <BsBoxArrowUpRight color={'blue'} size={20} /></button>
            <p>{props.desc}</p>
        </div>
    )
}
