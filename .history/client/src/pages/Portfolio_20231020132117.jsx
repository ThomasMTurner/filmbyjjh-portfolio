...
const PortfolioCard = (props) => {
    ...
    return (
        <div onClick={() => navigate(`/portfolio/${props.title.toLowerCase().slice(0, 5)}`)} onMouseEnter={() => setHovering(true)} onMouseLeave={() => setHovering(false)} className={`portfolio-card portfolio-card-${props.title.toLowerCase()}`}>
            <p style={{fontFamily:'helvetica', fontWeight:'bold', fontSize:'2.5rem', color:'white'}}>{props.title}</p>
            <button className={`portfolio-card-button ${hovering ? 'portfolio-card-button-hover' : ''}`} type="button" onClick={() => navigate(`/portfolio/${props.title.toLowerCase().slice(0, 5)}`)}>View more <BsBoxArrowUpRight color={'blue'} size={15} /></button>
            <p style={{fontFamily:'helvetica', fontWeight:'bold', fontSize:'1.5rem', color:'white'}}>{props.desc}</p>
        </div>
    )
}
